...
public class MemberDB extends AbstractDatabase<MemberBean> {
    ...
    public MemberDB(String url, String username, String password) {
        super(url, username, password, "user",
                "insert into user values(Default, ?, ?, ?, ?, ?, 0)",
                "UPDATE user set fname = ?, lname = ?, email = ?, phone = ?, pwd = ? where id = ?");
    }
    
    public MemberBean verify(String email, String password){
        final MemberBean bean = new MemberBean();
        tryHarder((SqlAction) () -> {
            st = new SqlStatement("select * from user where email = ? and pwd = ?", email, password);
            if(_query(st).status){
            if(st.rs.next())
                bean.update(st.rs);
            else
