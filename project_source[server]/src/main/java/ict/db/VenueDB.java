...
public class VenueDB extends AbstractDatabase<VenueBean> {
    ...
    protected SqlResultSet _queryByCol(String col, String value) {
        SqlResultSet rs = new SqlResultSet(tryHarder((SqlAction) () -> {
            st = new SqlStatement("select venue.*,CONCAT(user.lname, user.fname) as staffname, user.email as email from " + name + " inner join user on " + name + ".staffid = user.id where " + name + ".`" + col + "` = ?", value);
            st.executeQuery();
        }, st), st.rs);
        return rs;
    }
    ...
    protected SqlResultSet _queryByCol(String col, int value) {
        SqlResultSet rs = new SqlResultSet(tryHarder((SqlAction) () -> {
            st = new SqlStatement("select venue.*,CONCAT(user.lname, user.fname) as staffname, user.email as email from " + name + " inner join user on " + name + ".staffid = user.id where " + name + ".`" + col + "` = ?");
    ...
    @Override
    public ArrayList<VenueBean> readAll() {
